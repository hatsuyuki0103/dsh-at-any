/**
 * dsh-at-any client plugin: the browser half of the Codex-style @file
...
export function apply(ctx: ClientContext): void {
  ...
  const reportSettingsError = (
    ...
    console.error(`[dsh-at-any] settings ${operation} failed:`, error)
  }
  ...
  const search = async (sessionId: SessionId, signal: AbortSignal): Promise<readonly FileEntry[]> => {
    if (atFile === undefined) throw new Error('dsh-at-any: the atFile Remote is not mounted')
  ...
  ctx.slots.inject('conversation.input.overlay', () => ctx.slots.register({
    ...
    inject: (sessionId): FolderNavigatorInjected => {
      ...
      if (actx === undefined) throw new Error(`dsh-at-any: session "${String(sessionId)}" has no client scope`)
