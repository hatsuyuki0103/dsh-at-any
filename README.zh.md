...
**与 dsh-at-file 的区别**：索引不再有数量上限（默认收录全部源文件，不截断），**所有格式**的文件（.java、.vue、.ts、XML、YAML、图片、PDF、无扩展名文件、隐藏文件如 `.env` 等）都能被 @ 到；产物目录（`target`/`dist`/`node_modules` 等）仍默认跳过。
...
已有安装也使用这条命令更新。安装完成后重启 `dsh web`，确保 Host 和浏览器客户端加载 `0.1.0`。

> **替换 dsh-at-file**：本插件与 dsh-at-file 使用相同的 `atFile` 服务命名空间，二者不能同时启用。升级路径：先移除旧插件（编辑 profile 的 `package.json`，删除 `"dsh-at-file": "file:..."` 依赖行，或运行 `dsh plugin --profile web remove dsh-at-file`），再安装本插件并重启 `dsh web`。
...
- `maxIndexedFiles` 索引条目的软上限，**默认 1,000,000（实际上不截断）**。仅当工作区文件数超过该值时索引才会截断；普通项目不会触发。
