...
describe('dsh-at-any client apply', () => {
  ...
  it('logs a structured settings read failure and keeps the last snapshot', async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] settings read failed: SETTINGS_DOWN: unavailable')
  ...
  it('logs a rejecting settings read', async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] settings read failed:', expect.any(Error))
    } finally {
      errorSpy.mockRestore()
    }
  })
  ...
  it('logs settings updates rejected by the Remote', async () => {
    ...
    try {
      ...
      await settingsSection(booted).inject().setEnabled(false)
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] settings update failed: WRITE_REFUSED: read only')
      expect(settingsSection(booted).inject().hooks.scope.getSnapshot().value.enabled).toBe(true)
    } finally {
      errorSpy.mockRestore()
    }
  ...
  it('logs a rejecting settings update', async () => {
    ...
    try {
      ...
      await settingsSection(booted).inject().setEnabled(false)
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] settings update failed:', expect.any(Error))
    } finally {
      errorSpy.mockRestore()
    }
  })
  ...
  it('handles settings actions before the Remote mount settles', async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        '[dsh-at-any] settings update failed:',
  ...
  it('logs an open whose token has no index entry', async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] open failed: no index entry for', 'missing.ts')
    } finally {
      errorSpy.mockRestore()
    }
  })
  ...
  it('logs a rejecting host open', async () => {
    ...
    try {
      ...
      dock.inject('s1').onOpen('a.ts')
      await Promise.resolve()
      expect(errorSpy).toHaveBeenCalledWith('[dsh-at-any] open failed:', expect.any(Error))
    } finally {
      errorSpy.mockRestore()
    }
  })
