...
describe('dsh-at-any host composition', () => {
  ...
  it('search indexes the addressed workspace, files and directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsh-at-any-runtime-'))
  ...
  it('adds only the addressed workspace filters to the global list', async () => {
    const first = await mkdtemp(join(tmpdir(), 'dsh-at-any-runtime-first-'))
    const second = await mkdtemp(join(tmpdir(), 'dsh-at-any-runtime-second-'))
